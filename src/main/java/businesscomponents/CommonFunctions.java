...
public class CommonFunctions extends ReusableLibrary
{
	...
	public void clickOnElement(By identifer, String strElementName, String strElementType, String StatusFlag ) {
		...
		if(webElement.isDisplayed()){ webElement.click(); System.out.print("Pass-Click");
		//report.updateTestLog("Click on "+ strElementName , strElementName +" "+
		//		strElementType+" is clicked successfully", Status.valueOf(StatusFlag)); 
		} 
		else {
			System.out.print("Fail- Click");
			//report.updateTestLog("Verify element:", strElementName +": is not displayed",
			//		Status.FAIL); 
		}

	}

	public void verifyElement(By identifer, String strElementName, String strElementType, Status StatusFlag ) {
		...
		if(webElement.isDisplayed()){
			report.updateTestLog("Click on "+ strElementName ,  strElementName +" "+ strElementType+" is clicked successfully", StatusFlag);
		}
		else{
	...
	public void enterValuesInTextBox(By identifer,String strTextBoxName, String strValue, String StatusFlag ) {
		...
		if(webElement.isDisplayed()){
			...
			report.updateTestLog("Enter values in "+strTextBoxName, strValue +" is entered successfully", Status.valueOf(StatusFlag));
		}
		else {
	...
	public void browseAndUploadFile(By identifer,String strLocation, Status StatusFlag ){

		By strA = identifer;

		try{
			Assert.assertTrue(wait1.until(ExpectedConditions.visibilityOfElementLocated(strA)).isDisplayed());
			WebElement webElement= driver.findElement(strA);
			if(webElement.isDisplayed()){
				webElement.sendKeys(strLocation);
				report.updateTestLog("Browse file and uploadit", "File is browsed from location: "+strLocation +" and uploaded successfully", StatusFlag);
			}
		}
		catch(Exception e){
	...
	public void enterValues(By identifer,String strName, String strValue, Status StatusFlag ){
		By strA = identifer;
		waitForPageLoad(driver);
		wait1.until(ExpectedConditions.visibilityOfElementLocated(strA));
		WebElement webElement= driver.findElement(strA);
		if(webElement.isDisplayed()){
			webElement.clear();
			webElement.sendKeys(strValue);
			report.updateTestLog("Enter values in "+strName, strValue +" is entered successfully", StatusFlag);
		}
		else{
	...
	public void selectValueFromDropDown(By identifer,String strDropDownName, String strValue, String StatusFlag ) {
		...
		if(webElement.isDisplayed()){
			...
			report.updateTestLog("Select values from "+strDropDownName, strValue +" is selected successfully", Status.valueOf(StatusFlag));
		}
		else{
			report.updateTestLog("Select values from "+strDropDownName, strValue +" is not selected successfully", Status.FAIL);
	...
	waitForPageLoad(driver);
		...
		if(shadowContent.isDisplayed()){
			report.updateTestLog("Verify "+ strElementName ,  strElementName +" "+ strElementType+" is displayed successfully", Status.valueOf(StatusFlag));}
		else{
			report.updateTestLog("Verify element:", strElementName +": is not displayed", Status.FAIL);
		}
	...
	waitForPageLoad(driver);
		...
		if(shadowContent.isDisplayed()){
			shadowContent.click();
			report.updateTestLog("Click on "+ strElementName ,  strElementName +" "+ strElementType+" is clicked successfully", Status.valueOf(StatusFlag));}
		else{
			report.updateTestLog("Verify element:", strElementName +": is not displayed", Status.FAIL);
	...
	public void clickOnShadowElementWithScrollView(By ShadowHost, By ShadowRoot,  String strElementName, String strElementType, String StatusFlag ) {
		waitForPageLoad(driver);
		wait1.until(ExpectedConditions.visibilityOfElementLocated(ShadowHost));
		WebElement shadowHost = driver.findElement(ShadowHost);
		SearchContext shadowRoot = shadowHost.getShadowRoot();
	...
	public void clickOnNestedShadowElement(By ShadowHost, By ShadowRoot1,  By ShadowRoot2, String strElementName, String strElementType, String StatusFlag ) {
		waitForPageLoad(driver);
		wait1.until(ExpectedConditions.visibilityOfElementLocated(ShadowHost));
		WebElement shadowHost = driver.findElement(ShadowHost);
		SearchContext shadowRoot = shadowHost.getShadowRoot();
	...
	public void clickOnButton(By identifer, String strElementName, String StatusFlag ) {
		...
		if(webElement.isDisplayed()){ 
			webElement.click(); 
			report.updateTestLog("Click on "+ strElementName , strElementName +" button is clicked successfully", Status.valueOf(StatusFlag)); } 
		else {
			report.updateTestLog("Verify element:", strElementName +": is not displayed",
					Status.FAIL); 
		}
